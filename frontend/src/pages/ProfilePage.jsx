...
const ProfilePage = () => {
  ...
  const handleUpdate = async(e)=>{
    e.preventDefault();
    let res = await axios.put(`http://localhost:8080/users/update/${userDetails.user._id}`,userInfo,{
      headers:{
...
setuserInfo({
  ...
  const getuserPosts = async()=>{
    let res = await axios.get(`http://localhost:8080/posts/getYourPost/${userDetails.user._id}`,{
      headers:{
        'Authorization':userDetails.token
      }
    })
  ...
  //****************** */ profile and cover pic section starts here****************
  const [profilePicUpdater, setProfilePicUpdater] = useState('');
  const [coverPicUpdater, setcoverPicUpdater] = useState('');
 

 const handleProfilePicsChanger = (e)=>{
  
   setProfilePicUpdater(e.target.files[0])
    let reader = new FileReader();
    reader.readAsDataURL(e.target.files[0])
    reader.onload = async()=>{
      console.log(reader.result)
       let res = await axios.put(`http://localhost:8080/users/update/${userDetails.user._id}`,{profilePic:reader.result},{
      headers:{
        'Authorization':userDetails.token
      }
  ...
  const handleCoverPicsChanger = (e)=>{
   setcoverPicUpdater(e.target.files[0])
   let reader = new FileReader();
   reader.readAsDataURL(e.target.files[0])
   reader.onload = async()=>{
     console.log(reader.result)
      let res = await axios.put(`http://localhost:8080/users/update/${userDetails.user._id}`,{coverPic:reader.result},{
     headers:{
       'Authorization':userDetails.token
     }
  ...
  <img src={coverPicUpdater?URL.createObjectURL(coverPicUpdater) :userDetails?.user?.coverPic?userDetails.user?.coverPic: 'https://images.pexels.com/photos/3377405/pexels-photo-3377405.jpeg?cs=srgb&dl=pexels-elina-araja-1743227-3377405.jpg&fm=jpg'} className="w-full object-cover object-center h-60" />
  <input onChange={handleCoverPicsChanger} type="file" name='coverPicUpdater' id='coverPic' hidden />
  <label htmlFor="coverPic" className='absolute right-4 top-5'><FaCameraRetro className=' cursor-pointer' size={30}/></label>
  <div className=" flex justify-center -mt-8">
  <div className='rounded-full relative w-24 h-24 border-solid border-white border-2 -mt-3'>
    <input onChange={handleProfilePicsChanger} type="file"  name='profilePicUpdater' id='profilePic' hidden/>
    <label htmlFor="profilePic" className='absolute right-1 bottom-2'>  <FaCameraRetro className=' cursor-pointer' color='black' size={20}/></label>
  <img src={profilePicUpdater?URL.createObjectURL(profilePicUpdater) : userDetails?.user?.profilePic} className="rounded-full object-cover w-full h-full border-solid border-white border-2" />		
  </div>
  </div>
